    ...
    <script>
        ...
        network.on('beforeDrawing', function(ctx) {{
            try {{
                ...
                if (beforeDrawingErrors++ < 1) {{
                    showBootError('beforeDrawing error: ' + err.message + '\\n' + (err.stack || ''));
                }}
            }}
        }});
        ...
        network.on('stabilizationIterationsDone', function() {{
            try {{
                console.log('[dry-rot] stabilization done');
                network.setOptions({{
                    physics: {{
                        barnesHut: {{
                ...
                showBootError('stabilizationIterationsDone error: ' + err.message + '\\n' + (err.stack || ''));
            }}
        }});
